...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.l1 = nn.Linear(self.input_size+self.hidden_size, self.output_size)
        self.l2 = nn.Linear(self.input_size+self.hidden_size, self.hidden_size)
    ...
    def forward(self, input, hidden):
        ...
        together = torch.cat((input,hidden),1)

        first_to_out = self.l1(together)
        output=self.log_softmax(first_to_out)

        second_to_hidden = self.l2(together)
        hidden=self.tanh(second_to_hidden)
