...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, out_seq_len=None):
        ...
        enc_output = torch.clone(output)

        into_decode=torch.reshape(source[:,0],(-1,1))

        #Starting Seq.
        outputs=None

        if out_seq_len is None:
            range_len = source.size()[1]
        else:
            range_len = out_seq_len

        for i in range(range_len):

            output,hidden=self.decoder.forward(into_decode,hidden,encoder_outputs=enc_output,attention=self.attention)
            next_step=torch.reshape(torch.argmax(output,dim=1),(-1,1))
