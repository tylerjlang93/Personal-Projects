...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        Z1 = (self.weights['W1'].T@X.T).T + self.weights['b1']
        A1 = self.sigmoid(Z1)
        Z2 = (self.weights['W2'].T@A1.T).T + self.weights['b2']
        A2 = Z2 
        ...
        one_hot_y = np.zeros((p.shape[0],p.shape[1]))
        one_hot_y[range(len(y)),y] = 1
        dL_dZ = (p - one_hot_y)/len(y)

        W2_grad = A1.T.dot(dL_dZ)
        b2_grad = np.sum(dL_dZ.T,axis=1)

        layer1_dev = dL_dZ.dot(self.weights['W2'].T)*self.sigmoid_dev(Z1)
        W1_grad = np.array(X).T.dot(layer1_dev)
        b1_grad = np.sum(layer1_dev.T,axis=1)


        self.gradients['W1'] = W1_grad
        self.gradients['W2'] = W2_grad
        self.gradients['b1'] = b1_grad
        self.gradients['b2'] = b2_grad
