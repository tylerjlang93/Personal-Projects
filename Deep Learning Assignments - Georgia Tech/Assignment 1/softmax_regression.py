...
class SoftmaxRegression(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        '''
        Notes to self: relu_forward (A)  is size 64,10
                       u_forward    (Z)  is size 64,10
        '''
        one_hot_y = np.zeros((len(y),10))
        one_hot_y[range(len(y)),y] = 1
        dL_dZ = (p - one_hot_y)/len(y)
        dZ_dW = self.ReLU_dev(Z)
        self.gradients['W1'] = np.array(X).T.dot((dL_dZ)*dZ_dW)
