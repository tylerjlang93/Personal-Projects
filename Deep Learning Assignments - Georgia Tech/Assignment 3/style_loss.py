...
class StyleLoss(nn.Module):
    def gram_matrix(self, features, normalize=True):
        ...
        depth = features.size()[1]
        height = features.size()[2]
        width = features.size()[3]
        dims = height*width

        feature_loss = features.view(depth, dims)
        loss_first = feature_loss
        transposed = loss_first.transpose(1,0)
        g = torch.mm(loss_first,transposed )
    ...
    def forward(self, feats, style_layers, style_targets, style_weights):
        ...
        ##############################################################################
        z_s = torch.zeros(1)
        style = torch.autograd.Variable(z_s)
        ind = 0

        # Compute style loss for each desired feature layer and sum.
        for l in style_layers:
            gram_m = self.gram_matrix(feats[l])
            diff = (gram_m - style_targets[ind])
            sq_diff = diff**2
            squared_sum = torch.sum(sq_diff)
            new_weights_update = style_weights[ind] * squared_sum
            style  += new_weights_update
            ind +=1
